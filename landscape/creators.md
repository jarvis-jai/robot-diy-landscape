...
- **一句話定位**：前玩具設計師轉型的 YouTube maker，專注於 3D 列印機器人專案與開源設計
- **強項（≤3）**：
  1. 開源機器人設計（openDog、Robot X 等）專案完整公開
  2. 3D 列印機械結構設計經驗豐富（cycloidal drives、harmonic drives）
  3. 高度活躍的社群互動（Patreon 支持、合作專案與 Colin Furze 等知名 YouTuber 合作）
- **弱項/缺口（≤3，可觀察）**：
  1. 專案複雜度高，初學者門檻較高
  2. 無結構化課程，以專案為主的內容形式
  3. 主要偏機械設計，電子與軟體教學相對較少
- **活躍度訊號**：1.39M 訂閱者、786 部影片、最近更新 1 天前（2026-01-30 查證）
- **與我們的關聯（一句話）**：DIY 機器人專案的標杆創作者，其開源專案可作為進階學習資源
- **Link(s)**：https://www.youtube.com/@jamesbruton / https://patreon.com/XRobots
...
- **Type**：YouTube
- **Primary language**：English
- **一句話定位**：工程教育家，專注於馬達、機械設計和 DIY 專案的原理講解
- **強項（≤3）**：
  1. 深入淺出的工程原理教學（馬達類型、齒輪傳動、線性致動器）
  2. 高品質長影片，單片觀看次數常達百萬（如 Mecanum wheel 影片 540 萬次）
  3. 實作專案結合理論（正在製作 WALL-E 機器人系列）
- **弱項/缺口（≤3，可觀察）**：
  1. 更新頻率較低（約 1-2 個月一支影片）
  2. 偏重機械與馬達，微控制器程式內容較少
  3. 無配套網站提供詳細文字教學
- **活躍度訊號**：1.19M 訂閱者、149 部影片、觀看次數超過 7200 萬、最近更新 3 週前
- **與我們的關聯（一句話）**：想理解機器人機械原理的首選教育頻道
- **Link(s)**：https://www.youtube.com/@Jeremy_Fielding / https://jeremyfielding.com

---

### How To Mechatronics

- **Name**：How To Mechatronics (Dejan)
- **Type**：YouTube + Website
- **Primary language**：English
- **一句話定位**：機電一體化工程師創辦的 Arduino 專案與教學平台，影片搭配完整網站文章
- **強項（≤3）**：
  1. 影片與網站（howtomechatronics.com）完整搭配，提供電路圖和原始碼
  2. 3D 列印機械專案深度（cycloidal vs harmonic drive 測試、RC 坦克等）
  3. 清晰的專案導向教學，從原理到實作
- **弱項/缺口（≤3，可觀察）**：
  1. 影片數量相對較少（132 部）
  2. 更新頻率中等（數月一支深度影片）
  3. 無互動社群（如 Discord）
- **活躍度訊號**：719K 訂閱者、132 部影片、觀看次數超過 6200 萬、有 Patreon 頁面
- **與我們的關聯（一句話）**：Arduino 機器人專案的高品質教學資源，適合中階學習者
- **Link(s)**：https://www.youtube.com/@HowToMechatronics / https://howtomechatronics.com
...
- **Type**：YouTube + Website
- **Primary language**：English
- **一句話定位**：專注於 Arduino、Raspberry Pi、ESP32 的電子專案教學工作坊
- **強項（≤3）**：
  1. 詳盡的感測器與馬達控制教學（Stepper motor 教學為經典）
  2. 配套網站（dronebotworkshop.com）提供完整文字教程
  3. Reddit 社群評價高，影片詳細且用心製作
- **弱項/缺口（≤3，可觀察）**：
  1. 更新頻率較低（最近一年更新較少）
  2. 影片長度偏長（常超過 30 分鐘）
  3. 專注電子元件控制，完整機器人專案較少
- **活躍度訊號**：659K 訂閱者、176 部影片、最近更新 2024-09（Arduino Alvik Robot）
- **與我們的關聯（一句話）**：電子元件與微控制器基礎教學的可靠資源
- **Link(s)**：https://www.youtube.com/@Dronebotworkshop / https://dronebotworkshop.com
...
- **Type**：YouTube
- **Primary language**：English
- **一句話定位**：STEM 教育家，提供 Arduino 和 Raspberry Pi 系統性入門教學課程
- **強項（≤3）**：
  1. 完整系統性課程（Arduino R4 WiFi 100+ 課、Raspberry Pi Pico W 系列）
  2. 真正的「絕對初學者」友好（step-by-step，不假設先備知識）
  3. 持續更新維護（最近 18 小時前有新影片）
- **弱項/缺口（≤3，可觀察）**：
  1. 影片風格較傳統講課式，娛樂性較低
  2. 專注基礎教學，進階機器人專案較少
  3. 無配套網站（toptechboy.com 內容較舊）
- **活躍度訊號**：425K 訂閱者、986 部影片、觀看次數超過 4000 萬、持續活躍
- **與我們的關聯（一句話）**：初學者學習 Arduino/Raspberry Pi 的最佳系統性課程
- **Link(s)**：https://www.youtube.com/@paulmcwhorter / http://toptechboy.com

---

### Maker 101

- **Name**：Maker 101 (MertArduino)
- **Type**：YouTube
- **Primary language**：English
- **一句話定位**：中階 DIY 專案頻道，提供 Arduino、ESP32 的可複製專案教學
- **強項（≤3）**：
  1. 專案開源（Instructables 文章、MakerWorld 3D 模型）
  2. PCB 設計檔案公開（PCBWay 專案頁面）
  3. 實用與美觀兼具的專案（3D 列印檯燈、步行機器人）
- **弱項/缺口（≤3，可觀察）**：
  1. 無深度原理講解
  2. 影片較短，細節需參考外部資源
  3. 社群互動較少
- **活躍度訊號**：172K 訂閱者、199 部影片、觀看次數超過 2400 萬、2016 年起活躍
- **與我們的關聯（一句話）**：提供可直接複製的中階 DIY 專案範本
- **Link(s)**：https://www.youtube.com/@maker101io / https://www.instructables.com/member/MertArduino/instructables/

---

### Chris Annin (Annin Robotics)

- **Name**：Chris Annin
- **Type**：YouTube + Open Source Project
- **Primary language**：English
- **一句話定位**：AR4 開源 6 軸機器人手臂的創作者，提供工業級 DIY 機器人手臂解決方案
- **強項（≤3）**：
  1. 完整的開源工業機器人手臂（AR4 MK4：Arduino 控制器 + Python 介面）
  2. 活躍的用戶社群（Discord、用戶 build showcase）
  3. 持續迭代更新（MK3 → MK4，ROS 2 支援、NVIDIA Isaac Sim 整合）
- **弱項/缺口（≤3，可觀察）**：
  1. 專案成本較高（需要 3D 列印和購買零件）
  2. 入門門檻高，需要一定機械和電子經驗
  3. 頻道訂閱數相對較少（37.3K）
- **活躍度訊號**：37.3K 訂閱者、最近更新 3 週前、教育和 STEM 領域廣泛使用
- **與我們的關聯（一句話）**：想要 DIY 工業級機器人手臂的首選開源專案
- **Link(s)**：https://www.youtube.com/@anninrobotics / https://www.anninrobotics.com
...
- **Name**：GreatScott!
- **Type**：YouTube
- **Primary language**：English (德國創作者)
- **一句話定位**：電子專案 YouTuber，專注於電子學基礎和實用 DIY 電路專案
- **強項（≤3）**：
  1. 電子學基礎講解清晰（電源供應、PCB 設計、焊接）
  2. 影片節奏緊湊，資訊密度高
  3. 專案實用性強（DIY 點焊機、電動滑板等）
- **弱項/缺口（≤3，可觀察）**：
  1. 專注電子電路，機器人完整專案較少
  2. 最近更新頻率下降
  3. 無配套網站或詳細文字教程
- **活躍度訊號**：超過 200 萬訂閱者、知名電子 DIY 頻道
- **與我們的關聯（一句話）**：學習電子學基礎和電路設計的高品質資源
- **Link(s)**：https://www.youtube.com/@greatscottlab
